...
def fetch_cmdline_options():
    ...
    parser.add_argument('--historical_start_date', dest='historical_start_date', default='06/05/2022',
    ...
    start_date, end_date, historical_start_date, input_file, historical_input_file, holiday_file = args.start_date, args.end_date, args.historical_start_date, args.input_file, args.historical_input_file, args.holiday_file
    ...
    return start_date, end_date, historical_start_date, input_file, historical_input_file, holiday_file
 
#Fetching data from the excel
def fetch_input_data(input_file):
    df_weekly_data = pd.read_excel(input_file, sheet_name='env_data')  
    df_weekly_data['start_time'] = [d.time() for d in df_weekly_data['start_date']]
    df_weekly_data['end_time'] = [d.time() for d in df_weekly_data['end_date']]
    ...
    for env in environments:
        if env not in stats_dict: 
            pct_planned_downtime = (filtered_dataframe_24hr[(filtered_dataframe_24hr['Environment'] == env) 
                                    & (filtered_dataframe_24hr['Planned'] == 'Yes')]['downtime'].sum()/(1440*days_24hr)) *100 
            pct_unplanned_downtime = (filtered_dataframe_24hr[(filtered_dataframe_24hr['Environment'] == env)
                                    & (filtered_dataframe_24hr['Planned'] == 'No')]['downtime'].sum()/(1440*days_24hr))* 100 
        ...
        if env not in stats_dict_workday:
            pct_planned_downtime = (filtered_dataframe_workday[(filtered_dataframe_workday['Environment'] == env) 
                                    & (filtered_dataframe_workday['Planned'] == 'Yes')]['downtime_workday'].sum()/(1440*days_workday)) *100 
            pct_unplanned_downtime = (filtered_dataframe_workday[(filtered_dataframe_workday['Environment'] == env) 
                                    & (filtered_dataframe_workday['Planned'] == 'No')]['downtime_workday'].sum()/(1440*days_workday))* 100 
            pct_uptime = 100 - pct_planned_downtime - pct_unplanned_downtime
...
def calculate_statistics_historical(historical_df_24hr, historical_df_workday, environments):
    columns = ['Environment','Date', 'Uptime', 'Planned', 'Unplanned'] 
    
    #dictionary to collect statistics
    hist_stats_dict = defaultdict(list)
    hist_stats_dict_workday = defaultdict(list)
    unplanned_summary_stats = {}
    
    #Calculate the statistics for 24 hr
    historical_df_24hr['start_date']  = pd.to_datetime(historical_df_24hr['start_date'])
    historical_df_24hr = historical_df_24hr.groupby(['Environment','Planned','summary', pd.Grouper(key='start_date', freq='W-SUN')])['downtime'].sum().                     reset_index().sort_values('start_date')
    historical_df_24hr['start_date']  = historical_df_24hr['start_date'] - datetime.timedelta(days=6)
    historical_df_24hr['downtime'] = historical_df_24hr['downtime']/(1440*7) * 100
    for d in historical_df_24hr['start_date'].unique():
        for env in environments:
            planned = list(historical_df_24hr[(historical_df_24hr['start_date'] == d) 
                        & (historical_df_24hr['Environment'] == env) 
                        & (historical_df_24hr['Planned'] == 'Yes')]['downtime'])
            unplanned = list(historical_df_24hr[(historical_df_24hr['start_date'] == d) 
                        & (historical_df_24hr['Environment'] == env)
                        & (historical_df_24hr['Planned'] == 'No')]['downtime'])
            planned = round(planned[0], 2) if planned else 0
            unplanned = round(unplanned[0], 2) if unplanned else 0
            uptime = 100 - planned - unplanned

            unplanned_summary = (historical_df_24hr[(historical_df_24hr['Environment'] == env )
                                & (historical_df_24hr['Planned'] == 'No')][['start_date','summary']]).groupby(['start_date'], as_index=False).agg(lambda x: set(x))
            #print(unplanned_summary)
            hist_stats_dict[(d,env)]= [env, pd.to_datetime(d).date(), uptime, planned, unplanned]
            unplanned_summary_stats[env] = unplanned_summary

    hist_stats_plot_data_24hr = pd.DataFrame(hist_stats_dict.values(), columns=columns)
    
    #Calculate the stats for workday 
    historical_df_workday['start_date']  = pd.to_datetime(historical_df_workday['start_date'])
    historical_df_workday = historical_df_workday.groupby(['Environment','Planned', pd.Grouper(key='start_date', freq='W-SUN')])['downtime_workday'].sum().                                 reset_index().sort_values('start_date')
    historical_df_workday['start_date']  = historical_df_workday['start_date'] - datetime.timedelta(days=6)
    historical_df_workday['downtime_workday'] = historical_df_workday['downtime_workday']/(1440*7) * 100
        
    for d in historical_df_workday['start_date'].unique():
        for env in environments:
            planned = list(historical_df_workday[(historical_df_workday['start_date'] == d) 
                        & (historical_df_workday['Environment'] == env) 
                        & (historical_df_workday['Planned'] == 'Yes')]['downtime_workday'])
            unplanned = list(historical_df_workday[(historical_df_workday['start_date'] == d) 
                        & (historical_df_workday['Environment'] == env) 
                        & (historical_df_workday['Planned'] == 'No')]['downtime_workday'])
            planned = round(planned[0], 2) if planned else 0
            unplanned = round(unplanned[0], 2) if unplanned else 0
            uptime = 100 - planned - unplanned
            hist_stats_dict_workday[(d,env)]= [env, pd.to_datetime(d).date(), uptime, planned, unplanned]
            
    hist_stats_plot_data_workday = pd.DataFrame(hist_stats_dict_workday.values(), columns=columns)

    return hist_stats_plot_data_24hr, hist_stats_plot_data_workday, unplanned_summary_stats

#Generate historical plots
def generate_historical_plots(labels, colors, hist_stats_plot_data_24hr, hist_stats_plot_data_workday, unplanned_hist_summary_stats, env):
    rows = 2
    cols = 2
    fig1, ax1 = plt.subplots(rows, 2)
    text_content = []
    chart_data = hist_stats_plot_data_24hr
        ...
        for col in range(cols):
            if row >=1:
                ...
                if col >=1:
                    break
                text_content = []
                ax1[row, col].set_title('{0} {1}'.format(env, summary_title),x= 0.3 , y = 0.7,  fontsize=14, fontweight="bold")
                ...
                txt1 = ax1[row, col].text(0.05, 0.70, "".join(text_content), ha='left', va = 'top', fontsize=10, rotation=0, wrap=True, transform=ax1[row, col].transAxes)
                txt1._get_wrap_line_width = lambda : 800                
            else:
                ax1[row,col].set_title('{0}'.format(chart_title), fontsize=10, fontweight="bold")
                chart_data[chart_data['Environment'] == env].plot.bar(ax=ax1[row,col], x= 'Date', color = colors, stacked= True, legend = False,width = 0.3,                                                        rot = 40)
...
def generate_plots(labels, colors, stats_dict,stats_dict_workday, planned_summary_stats, unplanned_summary_stats, environments, start_date, end_date):
    ...
    for row in range(2, rows):
        for col, env in enumerate(environments):                  
                ...
                ax[row, col].axis('equal')
                ax[row, col].axis('off')
                txt = ax[row, col].text(0.05, 1, "{1} {0}".format("\n* ".join(summary_data[env][0]), "*" if summary_data[env][0] else ""), ha='left', va = 'top', fontsize=12, rotation=0, wrap=True, transform=ax[row, col].transAxes)
    ...
    fig.legend(labels=[f'{x}' for x in labels],bbox_to_anchor=(0.81, 0.96), columnspacing = 10,prop={'size': 12}, ncol = 3)
...
if __name__ == "__main__":
    ...
    #Weekly Plots
    df_weekly_data = fetch_input_data(input_file)
    filtered_dataframe_24hr, filtered_dataframe_workday, days_24hr, days_workday = filter_input_data(df_weekly_data, start_date, end_date, holiday_file)
    filtered_dataframe_24hr, filtered_dataframe_workday = calculate_downtime(filtered_dataframe_24hr, filtered_dataframe_workday)

    stats_dict, stats_dict_workday, planned_summary_stats, unplanned_summary_stats = calculate_statistics(filtered_dataframe_24hr, filtered_dataframe_workday, environments, days_24hr, days_workday)
    page1 = generate_plots(labels, colors, stats_dict,stats_dict_workday, planned_summary_stats, unplanned_summary_stats, environments, start_date, end_date)
    pages= [page1]
    
    #Historical Plots
    df_historical_data = fetch_input_data(historical_input_file)
    historical_df_24hr, historical_df_workday, historical_days_24hr, historical_days_workday = filter_input_data(df_historical_data, historical_start_date, end_date, holiday_file)
    historical_df_24hr, historical_df_workday = calculate_downtime(historical_df_24hr, historical_df_workday)
    hist_stats_plot_data_24hr, hist_stats_plot_data_workday, unplanned_hist_summary_stats = calculate_statistics_historical(historical_df_24hr, historical_df_workday, environments)
    
    #Add pages to final PDF file.
    for env in environments:
        page = generate_historical_plots(labels, colors, hist_stats_plot_data_24hr, hist_stats_plot_data_workday, unplanned_hist_summary_stats,env)
        pages.append(page)
